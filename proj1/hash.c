...
struct hash_elem* make_hashelem(int data) {
	struct hashitem* tmp;
...
void triple_action(struct hash_elem* elem, void* aux) {
    struct hashitem* tmph = hash_entry(elem, struct hashitem, elem);
    tmph->data = (tmph->data) * (tmph->data) * (tmph->data); // Triple
}

void square_action(struct hash_elem* elem, void* aux) {
    struct hashitem* tmph = hash_entry(elem, struct hashitem, elem);
    tmph->data = (tmph->data) * (tmph->data); // Square
}

// 소수를 사용하여 해싱
unsigned hash_int_2(int i) {
    unsigned prime = 103;
    unsigned hash_int = (unsigned) i % prime;
