...
#include "hex_dump.c"	// 'hex_dump' : defined in stdio.c(org:pintos/src/lib/)
...
bitmap_mark(struct bitmap* b, size_t bit_idx)
{
    ...
    asm("orq %1, %0" : "=m" (b->bits[idx]) : "r" (mask) : "cc");
}
...
bitmap_reset(struct bitmap* b, size_t bit_idx)
{
    ...
    asm("andq %1, %0" : "=m" (b->bits[idx]) : "r" (~mask) : "cc");
}
...
bitmap_flip(struct bitmap* b, size_t bit_idx)
{
    ...
    asm("xorq %1, %0" : "=m" (b->bits[idx]) : "r" (mask) : "cc");
}
...
struct bitmap* bitmap_expand(struct bitmap* bitmap, int size) {
    ...
    // 새로운 비트맵 할당
    struct bitmap* eb = malloc(sizeof * eb);
    if (eb == NULL) {
        return NULL;
    }

    int newsize = bitmap->bit_cnt + size;
    elem_type* new_bits = malloc(byte_cnt(newsize));
    if (new_bits == NULL) {
        free(eb);
        return NULL;
    }
    // 기존 비트 배열 복사
    memcpy(new_bits, bitmap->bits, byte_cnt(bitmap->bit_cnt));
    // 새로운 비트맵
    eb->bit_cnt = newsize;
    eb->bits = new_bits;
